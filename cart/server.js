...
app.get('/add/:id/:sku/:qty', (req, res) => {
    ...
    /*
     * CONTROLLED 404 TEST CASE
     *
     * Product HPTD should be visible on product page,
     * but adding it to cart should fail with real HTTP 404.
     *
     * This allows monitoring to capture backend 4xx
     * and frontend to redirect to custom /404 page.
     */
    if (sku === 'HPTD') {
        req.log.warn('Forced 404 while adding product to cart', {
            sku: sku,
            cartId: req.params.id
        });

        return res.status(404).send('404 - Product cannot be added to cart');
    }
